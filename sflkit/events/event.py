...
class Event(object):
    ...
    def __eq__(self, other):
        if isinstance(other, Event):
            return (self.file == other.file and self.line == other.line and
                    self.id_ == other.id_ and self.event_type == other.event_type)
        return False
...
class BranchEvent(Event):
    ...
    def deserialize(s: dict):
        ...
        return BranchEvent(*[s[p] for p in ['file', 'line', 'id_', 'then_id', 'else_id']])
...
class DefEvent(Event):
    ...
    def __init__(self, file, line: int, id_: int, var: str, var_id: int = None, value: Any = None, type_: str = None):
...
class FunctionEnterEvent(Event):
    ...
    def deserialize(s: dict):
        ...
        return FunctionEnterEvent(*[s[p] for p in ['file', 'line', 'id_', 'function_id', 'function']])


class FunctionExitEvent(Event):

    def __init__(self, file: str, line: int, id_: int, function_id: int, function: str, return_value: Any = None,
                 type_: str = None, tmp_var: str = None):
    ...
    def dump(self):
        return super().dump() + [self.function_id, self.function, self.return_value, self.type_]
...
class FunctionErrorEvent(Event):
    ...
    def deserialize(s: dict):
        ...
        return FunctionErrorEvent(*[s[p] for p in ['file', 'line', 'id_', 'function_id', 'function']])


class ConditionEvent(Event):

    def __init__(self, file: str, line: int, id_: int, condition: str, value: bool = None, tmp_var: str = None):
...
def load_event(event_type: EventType, *args):
    if event_type == EventType.BRANCH:
        return BranchEvent(args[0], int(args[1]), int(args[2]), int(args[3]), int(args[4]))
    elif event_type == EventType.DEF:
        try:
            return DefEvent(args[0], int(args[1]), int(args[2]), args[3], int(args[4]),
                            pickle.loads(base64.b64decode(args[5])), args[6])
        except:
            if args[5] == 'True':
                return DefEvent(args[0], int(args[1]), int(args[2]), args[3], int(args[4]),
                                True, args[6])
        ...
        return FunctionEnterEvent(args[0], int(args[1]), int(args[2]), int(args[3]), args[4])
    elif event_type == EventType.FUNCTION_EXIT:
        try:
            return FunctionExitEvent(args[0], int(args[1]), int(args[2]), int(args[3]), args[4],
                                     pickle.loads(base64.b64decode(args[5])), args[6])
        except:
            if args[5] == 'True':
                return FunctionExitEvent(args[0], int(args[1]), int(args[2]), int(args[3]), args[4],
        ...
        return FunctionErrorEvent(args[0], int(args[1]), int(args[2]), int(args[3]), args[4])
    elif event_type == EventType.CONDITION:
        return ConditionEvent(args[0], int(args[1]), int(args[2]), args[3], bool(int(args[4])))
    elif event_type in [EventType.LOOP_BEGIN, EventType.LOOP_HIT, event_type.LOOP_END]:
        return event_mapping[event_type](args[0], int(args[1]), int(args[2]), int(args[3]))
    else:
