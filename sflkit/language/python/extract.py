...
class PythonVarExtract(ast.NodeVisitor, VariableExtract):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> Any:
        ...
        if isinstance(node.value, ast.Name):
            return {f'{variable}.{node.attr}' for variable in variables if variable not in self.current_ignores}
        else:
            return variables
    ...
    def _visit_gen(self, node: Union[ast.ListComp, ast.SetComp, ast.DictComp, ast.GeneratorExp]):
        self.enter_ignores()
...
class PythonConditionExtract(ast.NodeVisitor, ConditionExtract):
    ...
    def __get_tmp_var(self, val: ast.AST, expression: str):
        ...
        e = ConditionEvent(self.file, val.lineno, self.factory.id_generator.get_next_id(), expression,
                           tmp_var=var)
        return var, ast.Name(id=var), ast.Module(
            body=[
                ast.Assign(
                    targets=[
                        ast.Name(
    ...
    def visit_BoolOp(self, node: ast.BoolOp) -> Any:
        ...
        final_var, final_use, final_assign, e = self.__get_tmp_var(
            ast.BoolOp(op=node.op, values=list(reversed(variables)), lineno=node.lineno),
            expression
        )
        es.append(e)
        return final_var, final_use, ast.Module(
            body=[
                assign,
                final_assign,
            ],
    ...
    def visit_UnaryOp(self, node: ast.UnaryOp) -> Any:
        if isinstance(node.op, ast.Not):
            ...
            final_var, final_use, final_assign, es = self.__get_tmp_var(
                ast.UnaryOp(op=node.op, operand=use, lineno=node.lineno),
                expression
            )
            return final_var, final_use, ast.Module(
                body=[
