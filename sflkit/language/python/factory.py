...
from sflkit.events.event import LineEvent, Event, BranchEvent, DefEvent, FunctionEnterEvent, FunctionErrorEvent, \
    FunctionExitEvent, LoopBeginEvent, LoopHitEvent, LoopEndEvent, UseEvent, ConditionEvent
from sflkit.language.meta import MetaVisitor, Injection, IDGenerator, TmpGenerator

python_lib = 'sflkit.instrumentation.lib'


def get_call(function, *args):
    return Expr(
        value=Call(
            func=Attribute(
                value=Name(
            ...
            args=[
                Constant(value=argument, ) for argument in args
            ],
            keywords=[],
        ),
    )
...
class PythonEventFactory(MetaVisitor, NodeVisitor):
    ...
    def __init__(self, language, id_generator: IDGenerator, tmp_generator: TmpGenerator):
...
class BranchEventFactory(PythonEventFactory):
    ...
    def get_event_call(self, event: BranchEvent):
        return get_call(self.get_function(), event.file, event.line, event.id_, event.then_id, event.else_id)

    def get_function(self):
        return 'add_branch_event'

    def _get_branch_events(self, node):
        then_branch_event = BranchEvent(self.file, node.lineno, self.id_generator.get_next_id(), self.branch_id,
                                        self.branch_id + 1)
        else_branch_event = BranchEvent(self.file, node.lineno, self.id_generator.get_next_id(), self.branch_id + 1,
                                        self.branch_id)
        self.branch_id += 2
        return then_branch_event, else_branch_event

    def _visit_branch(self, node: typing.Union[For, AsyncFor]) -> Injection:
        then_branch_event, else_branch_event = self._get_branch_events(node)
        return Injection(body=[self.get_event_call(then_branch_event)],
    ...
    def visit_ExceptHandler(self, node: ExceptHandler) -> Injection:
        branch_event = BranchEvent(self.file, node.lineno, self.id_generator.get_next_id(), self.branch_id, -1)
        return Injection(body=[self.get_event_call(branch_event)], events=[branch_event])

    def visit_Try(self, node: Try) -> Injection:
        else_branch_event = BranchEvent(self.file, node.lineno, self.id_generator.get_next_id(), self.branch_id, -1)
        return Injection(orelse=[self.get_event_call(else_branch_event)], events=[else_branch_event])
...
class DefEventFactory(PythonEventFactory):
    ...
    def get_event_call(self, event: DefEvent):
        call = get_call(self.get_function(), event.file, event.line, event.id_, event.var)
    ...
    def visit_function(self, node: typing.Union[FunctionDef, AsyncFunctionDef]) -> Injection:
        def_events = list()
        for argument in self.variable_extract.visit(node.args):
            if argument != 'self':
                def_events.append(DefEvent(self.file, node.lineno, self.id_generator.get_next_id(), argument))
        return Injection(body=[self.get_event_call(e) for e in def_events], events=[def_events])
    ...
    def visit_var_assign(self, node: typing.Union[Assign, AnnAssign, AugAssign], vars_: list) -> Injection:
        def_events = list()
        for var in vars_:
            def_events.append(DefEvent(self.file, node.lineno, self.id_generator.get_next_id(), var))
        return Injection(post=[self.get_event_call(e) for e in def_events], events=[def_events])

    def visit_Assign(self, node: Assign) -> Injection:
        vars_ = sum([list(self.variable_extract.visit(target)) for target in node.targets], start=list())
...
class FunctionEnterEventFactory(FunctionEventFactory):
    ...
    def get_event_call(self, event: FunctionEnterEvent):
        return get_call(self.get_function(), event.file, event.line, event.id_, event.function_id, event.function)

    def visit_function(self, node: typing.Union[FunctionDef, AsyncFunctionDef]) -> Injection:
        function_enter_event = FunctionEnterEvent(self.file, node.lineno, self.id_generator.get_next_id(),
                                                  self.get_function_id(node), node.name)
        function_var = self.get_function_var(node, self.tmp_generator)
        return Injection(body=[self.get_event_call(function_enter_event), Assign(
            targets=[
                ast.Name(
                    id=function_var,
                ),
...
class FunctionExitEventFactor(FunctionEventFactory):
    ...
    def get_event_call(self, event: FunctionExitEvent):
        call = get_call(self.get_function(), event.file, event.line, event.id_, event.function_id, event.function)
    ...
    def visit_function(self, node: typing.Union[FunctionDef, AsyncFunctionDef]) -> Injection:
        function_exit_event = FunctionExitEvent(self.file, node.lineno,
                                                self.get_function_event_id(node, self.id_generator),
                                                self.get_function_id(node), node.name,
                                                tmp_var=self.get_function_var(node, self.tmp_generator))
        return Injection(body_last=[self.get_event_call(function_exit_event)], events=[function_exit_event])
    ...
    def visit_Return(self, node: Return) -> Injection:
        ...
        function_exit_event = FunctionExitEvent(self.file, node.lineno,
                                                self.get_function_event_id(node, self.id_generator),
                                                self.get_function_id(function), function.name,
        ...
        return Injection(
            pre=[
                ...
                self.get_event_call(function_exit_event)], assign=ast.Name(id=function_var),
...
class FunctionErrorEventFactor(FunctionEventFactory):
    ...
    def visit_function(self, node: typing.Union[FunctionDef, AsyncFunctionDef]) -> Injection:
        function_error_event = FunctionErrorEvent(self.file, node.lineno,
                                                  self.get_function_event_id(node, self.id_generator),
                                                  self.get_function_id(node), node.name)
        return Injection(error=[self.get_event_call(function_error_event)], events=[function_error_event])
...
class LoopEventFactory(PythonEventFactory):
    ...
    def get_event_call(self, event: typing.Union[LoopBeginEvent, LoopHitEvent, LoopEndEvent]):
        return get_call(self.get_function(), event.file, event.line, event.id_, event.loop_id)
...
class LoopBeginEventFactory(LoopEventFactory):
    ...
    def visit_loop(self, node: typing.Union[For, AsyncFor, While]) -> Injection:
        loop_begin_event = LoopBeginEvent(self.file, node.lineno, self.id_generator.get_next_id(),
                                          self.get_loop_id(node))
        return Injection(pre=[self.get_event_call(loop_begin_event)], events=[loop_begin_event])
...
class LoopHitEventFactory(LoopEventFactory):
    ...
    def visit_loop(self, node: typing.Union[For, AsyncFor, While]) -> Injection:
        loop_hit_event = LoopHitEvent(self.file, node.lineno, self.id_generator.get_next_id(),
                                      self.get_loop_id(node))
        return Injection(body=[self.get_event_call(loop_hit_event)], events=[loop_hit_event])
...
class LoopEndEventFactory(LoopEventFactory):
    ...
    def visit_loop(self, node: typing.Union[For, AsyncFor, While]) -> Injection:
        loop_end_event = LoopEndEvent(self.file, node.lineno, self.id_generator.get_next_id(),
                                      self.get_loop_id(node))
        return Injection(finalbody=[self.get_event_call(loop_end_event)], events=[loop_end_event])
...
class UseEventFactory(PythonEventFactory):
    ...
    def _get_try_wrapper(self, event: UseEvent):
        return Try(
            body=[
                self._get_wrapped_property(event)
            ],
            handlers=[
                ExceptHandler(
                    type=Tuple(
    ...
    def visit_use(self, node: AST):
        ...
        use_events = list()
        for use in uses:
            use_events.append(UseEvent(self.file, node.lineno, self.id_generator.get_next_id(), use))
        return Injection(pre=[self.get_event_call(e) for e in use_events], events=[use_events])
    ...
    def visit_With(self, node: With) -> Injection:
        return sum([self.visit_use(item.context_expr) for item in node.items], start=Injection())

    def visit_AsyncWith(self, node: AsyncWith) -> Injection:
        return sum([self.visit_use(item.context_expr) for item in node.items], start=Injection())

    def visit_Delete(self, node: Delete) -> Injection:
        return sum([self.visit_use(target) for target in node.targets], start=Injection())
...
class ConditionEventFactory(PythonEventFactory):
    ...
    def get_event_call(self, event: ConditionEvent):
        return get_call(self.get_function(), event.file, event.line, event.id_, event.tmp_var, event.condition)
