...
class DirInstrumentation(Instrumentation):
    ...
    def instrument(self, src: str, dst: str, suffixes: List[str] = None, file: str = '', excludes: list = None):
        if suffixes is None:
        ...
        if os.path.isfile(src):
            ...
            self.file_instrumentation.instrument(src, dst, suffixes=suffixes, file=os.path.split(src)[-1])
        else:
            ...
            while not file_queue.empty():
                ...
                if any(re.match(exclude, element) for exclude in excludes):
                    if os.path.isdir(os.path.join(src, element)):
                        shutil.copytree(os.path.join(src, element), os.path.join(dst, element), symlinks=True)
                    else:
                        shutil.copy(os.path.join(src, element), os.path.join(dst, element), follow_symlinks=False)
                    continue
                if os.path.isdir(os.path.join(src, element)):
                ...
                elif any(element.endswith(f'.{suffix}') for suffix in suffixes) and not os.path.islink(
                        os.path.join(src, element)):
                    logging.debug(f'I found a file I can instrument at {element}.')
                    self.file_instrumentation.instrument(os.path.join(src, element), os.path.join(dst, element),
                                                         file=element)
                else:
