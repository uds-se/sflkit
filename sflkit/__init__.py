...
def instrument_config(conf: Config, event_dump: str = None):
    ...
    instrumentation.instrument(conf.target_path, conf.instrument_working, suffixes=conf.language.suffixes,
...
def analyze_config(conf: Config, analysis_dump: str = None):
    ...
    for analysis_type in conf.predicates:
        ...
        for metric in conf.metrics + [Predicate.IncreaseTrue, Predicate.IncreaseFalse]:
            try:
                results[analysis_type.name][metric.__name__] = analyzer.get_sorted_suggestions(conf.target_path, metric,
                                                                                          analysis_type)
...
__all__ = ['analysis', 'events', 'instrument', 'language', 'model', 'runners', 'config', 'instrument',
           'instrument_config', 'analyze', 'analyze_config']
