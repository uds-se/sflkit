...
from sflkit.analysis.predicate import Branch, Condition, Comp, ScalarPair, VariablePredicate, ReturnPredicate, \
    NonePredicate, EmptyStringPredicate, IsAsciiPredicate, ContainsDigitPredicate, ContainsSpecialPredicate, \
    EmptyBytesPredicate
...
class CombinationFactory(AnalysisFactory):
    ...
    def get_analysis(self, event, scope: Scope = None) -> List[AnalysisObject]:
        return sum([factory.handle(event, scope) for factory in self.factories], start=list())
...
class BranchFactory(AnalysisFactory):
    ...
    def get_analysis(self, event, scope: Scope = None) -> List[AnalysisObject]:
        if event.event_type == EventType.BRANCH:
            ...
            if event.else_id >= 0:
                else_key = (Branch.analysis_type(), event.file, event.line, event.else_id)
...
class LoopFactory(AnalysisFactory):
    ...
    def get_analysis(self, event, scope: Scope = None) -> List[AnalysisObject]:
        if event.event_type in (EventType.LOOP_BEGIN, EventType.LOOP_HIT, EventType.LOOP_END):
    ...
    def get_analysis(self, event, scope: Scope = None) -> List[AnalysisObject]:
        if event.event_type == EventType.DEF:
            ...
            if event.var_id in self.id_to_def:
                key = (DefUse.analysis_type(), self.id_to_def[event.var_id].file, self.id_to_def[event.var_id].line,
                       event.file, event.line, event.var)
                if key not in self.objects:
                        ...
                        for variable in variables:
                            if variable.var != event.var:
                                if variable.type_ in types:
                                    for comp in Comp:
                                        key = (ScalarPair.analysis_type(), event.file, event.line,
                                               event.var, variable.var, comp, types[0])
                                        if key not in self.objects:
                                            self.objects[key] = ScalarPair(event, comp, variable.var)
                ...
                for variable in variables:
                    if variable.type_ == event.type_:
                        for comp in (Comp.EQ, Comp.NE):
                            key = (ScalarPair.analysis_type(), event.file, event.line,
                                   event.var, variable.var, comp, event.type_)
                            if key not in self.objects:
                                self.objects[key] = ScalarPair(event, comp, variable.var)
                            objects.append(self.objects[key])
            return objects
...
class VariableFactory(AnalysisFactory):
    ...
    def get_analysis(self, event, scope: Scope = None) -> List[AnalysisObject]:
        if event.event_type == EventType.DEF and event.type_ in ['int', 'float', 'bool']:
            objects = list()
            for comp in Comp:
                key = (VariablePredicate.analysis_type(), event.file, event.line,
                       event.var, comp, 'int')
                if key not in self.objects:
        ...
        if event.event_type == EventType.FUNCTION_EXIT:
            ...
            if event.type_ in ('int', 'float', 'bool', 'str', 'bytes'):
                if event.type_ in ('int', 'float', 'bool'):
                    ...
                    compare = Comp.EQ, Comp.NE
                for comp in compare:
                    key = (ReturnPredicate.analysis_type(), event.file, event.line,
                           event.function, comp, type_)
                    if key not in self.objects:
                        self.objects[key] = ReturnPredicate(event, comp, true_relevant=tr)
                    objects.append(self.objects[key])
            if event.type_ == 'NoneType':
                for comp in Comp.EQ, Comp.NE:
                    key = (ReturnPredicate.analysis_type(), event.file, event.line,
                           event.function, comp, event.event_type_)
                    if key not in self.objects:
                        self.objects[key] = ReturnPredicate(event, comp, true_relevant=None)
                    objects.append(self.objects[key])
            else:
                for comp in Comp.EQ, Comp.NE:
                    key = (ReturnPredicate.analysis_type(), event.file, event.line,
                           event.function, comp, 'NoneType')
...
class ConstantCompFactory(AnalysisFactory):
    ...
    def get_analysis(self, event, scope: Scope = None) -> List[AnalysisObject]:
        if event.event_type == EventType.DEF:
            objects = list()
            for comp in Comp.EQ, Comp.NE:
                key = (self.class_.analysis_type(), event.file, event.line,
                       event.var, comp)
                if key in self.objects:
