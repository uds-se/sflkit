...
class Predicate(Spectrum):
    ...
    def evaluate(self, failed: bool = False, res: bool = False, scope_: scope.Scope = None):
        ...
        self._handle_predicate(failed, res or self._evaluate_predicate(scope_ if scope_ else dict()))
    ...
    def Fail(self) -> Tuple[float, float]:
        try:
            self.fail_true = self.true_relevant / (self.true_relevant + self.true_irrelevant)
        except ZeroDivisionError:
            pass
        try:
            self.fail_false = self.false_relevant / (self.false_relevant + self.false_irrelevant)
        except ZeroDivisionError:
            pass
    ...
    def Contex(self) -> float:
        try:
            self.context = (self.true_relevant + self.false_relevant) / (self.true_relevant + self.true_irrelevant
...
class Comparison(Predicate):
    ...
    @staticmethod
    def events():
        return [EventType.DEF, EventType.FUNCTION_ENTER, EventType.FUNCTION_EXIT, EventType.FUNCTION_ERROR]
...
class ReturnPredicate(Comparison):
    ...
    def __init__(self, event: FunctionExitEvent, op: Comp, true_relevant: int = 0):
        super().__init__(event.file, event.line, true_relevant)
        self.op = op
