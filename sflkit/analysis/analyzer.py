...
class Analyzer(object):
    ...
    def __init__(self, relevant_event_files: List[EventFile], irrelevant_event_files: List[EventFile],
    ...
    def get_analysis_by_type(self, type_: AnalysisType):
        return list(filter(lambda p: p.analysis_type() == type_, self.model.get_analysis()))

    def get_sorted_suggestions(self, base_dir, metric: Callable = None, type_: AnalysisType = None):
        if type_:
            objects = self.get_analysis_by_type(type_)
        else:
            objects = self.get_analysis()
        return sorted(map(lambda p: p.get_suggestion(metric=metric, base_dir=base_dir), objects), reverse=True)[:]
